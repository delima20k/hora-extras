...
describe('DayController', () => {
  const createController = () => {
    const entries = []; const repository = { findByDate: vi.fn(async () => entries.filter((entry) => entry.status === 'active')), findByMonth: vi.fn(async () => entries.filter((entry) => entry.status === 'active')), findAll: vi.fn(async () => entries.filter((entry) => entry.status === 'active')), create: vi.fn(async (entry) => entries.push(entry.toObject())), update: vi.fn(async (entry) => { const index = entries.findIndex((item) => item.id === entry.id); entries[index] = entry.toObject(); }), delete: vi.fn(async (entry) => { const index = entries.findIndex((item) => item.id === entry.id); entries[index] = new OvertimeEntry(entries[index]).softDelete().toObject(); }) };
    const state = { selectedDate: '2026-07-15', employee: { id: 'employee-1' }, payrollSettings: { salary: 2200, monthlyWorkload: 220 }, workSchedule: { workDays: ['monday', 'tuesday', 'wednesday', 'thursday', 'friday'], startTime: '06:00', endTime: '14:20' } }; const view = { renderLoading: vi.fn(), render: vi.fn(), updateDurationPreview: vi.fn() }; return { controller: new DayController({ state, dateService: {}, entryRepository: repository }), repository, view };
  };
