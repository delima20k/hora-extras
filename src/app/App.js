...
export class App {
  ...
  async initialize() {
    ...
    try {
      ...
      this.profileController = new ProfileController({ profileRepository: this.repositories.profile, imageService: this.imageService, onSaved: async (bundle) => { Object.assign(this.state, bundle); this.updateAvatarUrl(); this.refreshChrome(); if (this.state.currentRoute === 'total') await this.totalController.refresh(); } });
      ...
      this.dayController = new DayController({ state: this.state, dateService: this.dateService, entryRepository: this.repositories.overtimeEntry, timeCalculationService: new TimeCalculationService(), onEntriesChanged: () => { if (this.state.currentRoute === 'month') this.calendarController.render(); if (this.state.currentRoute === 'total') this.totalController.refresh(); } });
      this.navigation = new NavigationController({ layout: this.layout, state: this.state, dateService: this.dateService, onNavigate: () => { this.closeMenu(); this.renderSidebar(); }, calendarController: this.calendarController, dayController: this.dayController, totalController: this.totalController, profileActions: { onSubmit: (...args) => this.profileController.submit(...args), onFileChange: (...args) => this.profileController.handleFile(...args) } });
