...
export class CalendarView {
  ...
  renderMonth(model, handlers) {
    ...
    const weekdayRow = element('div', { className: 'calendar-weekdays', 'aria-hidden': 'true' }, model.weekdayLabels.map((label) => element('span', { text: label })));
    const grid = element('div', { className: 'calendar-grid', 'aria-label': `Calendário de ${model.title}` });
    model.weeks.flat().forEach((day) => {
      ...
      const dayButton = button(String(day.day), { className: classes.join(' '), 'data-date': day.date, 'aria-label': this.calendarService.getAriaLabel(day), 'aria-pressed': String(day.isSelected), onClick: () => handlers.onDaySelected(day) });
      this.dayButtons.set(day.date, dayButton);
      grid.append(dayButton);
    });
    this.container.replaceChildren(navigation, weekdayRow, grid);
  }
  updateSelection(previousDate, selectedDay) {
    const previous = this.dayButtons.get(previousDate);
    if (previous) { previous.classList.remove('is-selected'); previous.setAttribute('aria-pressed', 'false'); }
    const current = this.dayButtons.get(selectedDay.date);
    if (current) { current.classList.add('is-selected'); current.setAttribute('aria-pressed', 'true'); }
  }
}
