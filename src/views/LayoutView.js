...
export class LayoutView {
  ...
  render(onMenu) {
    ...
    const avatar = element('img', { className: 'header-avatar', src: '/default-avatar.svg', alt: 'Avatar do usuário' });
  ...
  updateHeader(title, employee, avatarUrl) { this.refs.title.textContent = title; this.refs.avatar.src = avatarUrl || '/default-avatar.svg'; this.refs.avatar.alt = `Avatar de ${displayName(employee)}`; }
  setMenuOpen(open) { this.refs.menuButton.setAttribute('aria-expanded', String(open)); this.refs.menuButton.setAttribute('aria-label', open ? 'Fechar menu' : 'Abrir menu'); this.refs.sidebar.setAttribute('aria-hidden', String(!open)); this.refs.sidebar.classList.toggle('is-open', open); this.refs.overlay.hidden = !open; this.refs.overlay.classList.toggle('is-open', open); }
}
