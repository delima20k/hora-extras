...
export class SidebarView {
  render(container, { employee, avatarUrl, currentRoute }, onNavigate) {
    ...
    Object.entries(ROUTES).forEach(([key, title]) => nav.append(button(title, { className: `nav-item${currentRoute === key ? ' is-active' : ''}`, 'aria-current': currentRoute === key ? 'page' : undefined, onClick: () => onNavigate(key) })));
