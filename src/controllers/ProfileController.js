...
export class ProfileController {
  constructor({ profileRepository, imageService, onSaved }) {
  ...
  async handleFile(file, image, status) {
    try {
      if (!file) return;
      const blob = await this.imageService.process(file);
      this.pendingAvatar = blob;
      this.imageService.revokePreview(this.previewUrl);
      this.previewUrl = this.imageService.createPreview(blob);
      image.src = this.previewUrl;
      status.textContent = 'Imagem pronta para salvar.';
