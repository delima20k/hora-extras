...
export class PayrollClosureService {
  ...
  summarize(entries, payrollSettings) {
    return entries.reduce((total, entry) => {
      ...
      const nightPay = this.timeCalculationService.calculateOvertimePay(0, entry.date, payrollSettings, nightMinutes);
      const value = this.timeCalculationService.calculateOvertimePay(entry.durationMinutes, entry.date, payrollSettings, nightMinutes);
      total.totalMinutes += entry.durationMinutes; total.totalPay += value;
      total.nightMinutes += nightMinutes; total.nightPay += nightPay;
      if (this.timeCalculationService.getOvertimeMultiplier(entry.date) === 2) { total.minutes100 += entry.durationMinutes; total.value100 += value - nightPay; } else { total.minutes65 += entry.durationMinutes; total.value65 += value - nightPay; }
      return total;
